...
import { IconPhoto, IconTrash, IconUser, IconUpload, IconFilePlus, IconCamera } from "@tabler/icons-react";
...
export function QuickExamineeImportPanel({
  ...
  const [selectedBatch, setSelectedBatch] = useState<string | null>(initialBatchId ? String(initialBatchId) : null);
  ...
  useEffect(() => {
    ...
    const newCards: ParsedExaminee[] = lines.map((line, index) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      saveDraft(storageKey, { text, files: attachedFiles, batchId: selectedBatch });
    }, 1000);
  ...
  const processFiles = (files: File[]) => {
      ...
      if (newNames.length > 0) {
          const prefix = text && text.length > 0 && !text.endsWith("\n") ? "\n" : "";
          setText(prev => prev + prefix + newNames.join("\n"));
          notifications.show({ title: "Imported", message: `Added ${newNames.length} new participants`, color: 'blue' });
      } else {
          notifications.show({ title: "Updated", message: `Updated ${Object.keys(updates).length} existing participants`, color: 'teal' });
      }
  };
  ...
  return (
    <Stack h="100%">
       ...
       <Dropzone
          ...
          styles={{ inner: { flex: 1, display: 'flex', flexDirection: 'column' }}}
        >
        <SimpleGrid cols={2} spacing="md" style={{ flex: 1, minHeight: 0 }}>
            <Stack h="100%">
               <Group justify="space-between">
                   <Text fw={500}>Daftar Nama</Text>
                   <Text size="xs" c="dimmed">Satu nama per baris</Text>
               </Group>
               <Textarea
                  ...
                  style={{ flex: 1, display: 'flex', flexDirection: 'column' }}
                  styles={{ wrapper: { flex: 1 }, input: { height: '100%', fontFamily: 'monospace' } }}
               />
            </Stack>
            ...
            <Stack h="100%" style={{ overflowY: 'hidden' }}>
               ...
               <ScrollArea style={{ flex: 1 }} type="auto">
                  <Stack gap="sm" pb="md">
                      {cards.length === 0 && <Text c="dimmed" ta="center" mt="xl">Belum ada data preview</Text>}
                      {cards.map((card) => (
                          <Paper key={card.id} p="xs" withBorder radius="md">
                             <Group>
                                <Avatar src={card.imagePreviewUrl} radius="xl" size="md" color="initials">
                                    {card.name.charAt(0)}
                                </Avatar>
                                <Box style={{ flex: 1 }}>
                                    <Text size="sm" fw={500}>{card.name}</Text>
                                    <Group gap={5}>
                                        <Badge size="xs" variant="light" color={card.imageFile ? "green" : "gray"}>
                                           {card.imageFile ? "Ada Foto" : "Tanpa Foto"}
                                        </Badge>
                                        <Text size="xs" c="dimmed">Line {card.index + 1}</Text>
                                    </Group>
                                </Box>
                                ...
                                >
                                    {card.imageFile ? <IconUpload size={16}/> : <IconCamera size={16}/>}
                                </ActionIcon>
        ...
        <Group justify="flex-end">
           ...
           <Button onClick={handleSave} loading={isSaving} disabled={cards.length === 0}>
              Simpan ({cards.length})
           </Button>
        </Group>
    </Stack>
