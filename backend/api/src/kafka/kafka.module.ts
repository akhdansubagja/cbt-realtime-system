...
@Module({
  imports: [
    ClientsModule.registerAsync([ // <-- Gunakan registerAsync
      {
        name: 'KAFKA_SERVICE',
        imports: [ConfigModule], // <-- Beritahu bahwa kita butuh ConfigModule
        useFactory: async (configService: ConfigService) => ({ // <-- Gunakan factory
          ...
          options: {
            client: {
              ...
              brokers: [
                configService.get<string>('KAFKA_BROKER_URL', 'localhost:29092'),
              ],
            },
            consumer: {
