...
describe('ExamineesController', () => {
  ...
  describe('findAllSimple', () => {
    it('should return a simple list of examinees', async () => {
      ...
      // Mock repo find
      (
        mockExamineesService.examineeRepository.find as jest.Mock
      ).mockResolvedValue(simpleList);

      const result = await controller.findAllSimple();

      expect(result).toEqual(simpleList);
      expect(mockExamineesService.examineeRepository.find).toHaveBeenCalled();
    });
  });
  ...
  describe('update', () => {
    it('should update an examinee', async () => {
      ...
      const result = await controller.update(examineeId, updateDto, mockFile);
